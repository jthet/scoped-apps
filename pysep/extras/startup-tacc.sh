if [ $# -eq 0 ]; then
    mkdir -p $(pwd)/scoped/
    cp -r /home/scoped/pysep/ $(pwd)/scoped/ >/dev/null 2>&1

    chmod 777 $(pwd)/scoped/

    export PATH=$PATH:$HOME/.local/bin # Adds pysep tools to shell
    source ~/.bashrc
    ...
    echo "Welcome to the pysep for Tapis" | sed  -e :a -e "s/^.\{1,$(tput cols)\}$/ & /;ta" | tr -d '\n' | head -c $(tput cols); echo -e "\n"
    echo  "The Link below will open a pysep Jupyter Notebook" | sed  -e :a -e "s/^.\{1,$(tput cols)\}$/ & /;ta" | tr -d '\n' | head -c $(tput cols); echo -e "\n"
