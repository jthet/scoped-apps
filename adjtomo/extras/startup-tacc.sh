if [ $# -eq 0 ]; then
    mkdir -p $(pwd)/scoped/adjtomo
    cp -r /home/scoped/pysep/ $(pwd)/scoped/adjtomo/ >/dev/null 2>&1
    cp -r /home/scoped/adjdocs/ $(pwd)/scoped/adjtomo/ >/dev/null 2>&1
    cp -r /home/scoped/seisflows/ $(pwd)/scoped/adjtomo/ >/dev/null 2>&1
    cp -r /home/scoped/pyatoa/ $(pwd)/scoped/adjtomo/ >/dev/null 2>&1
    cp -r /home/scoped/specfem2d/ $(pwd)/scoped/adjtomo/ >/dev/null 2>&1
    cp -r /home/scoped/specfem3d/ $(pwd)/scoped/adjtomo/ >/dev/null 2>&1

    chmod 777 $(pwd)/scoped/

    export PATH=$PATH:$HOME/.local/bin # Adds adjtomo tools to shell
    source ~/.bashrc
    ...
    echo "Welcome to the adjtomo for Tapis" | sed  -e :a -e "s/^.\{1,$(tput cols)\}$/ & /;ta" | tr -d '\n' | head -c $(tput cols); echo -e "\n"
    echo  "The Link below will open a adjtomo Jupyter Notebook" | sed  -e :a -e "s/^.\{1,$(tput cols)\}$/ & /;ta" | tr -d '\n' | head -c $(tput cols); echo -e "\n"
    ...
    PID=$!
    sleep 2400s;
    # End jupyter nb after 40 minutes
    # Change to environment variable

    # find $(pwd)/scoped/pysep -mindepth 1 -not -name "OUTPUT_FILES" -not -path "$(pwd)/scoped/pysep/OUTPUT_FILES/*" -exec rm -rf {} +
else
    exec $@
fi