if [ $# -eq 0 ]; then
    mkdir -p $(pwd)/scoped/
    cp -r /home/scoped/specfem3d/ $(pwd)/scoped/
    ...
    echo "Welcome to the SPECFEM3D for Tapis" | sed  -e :a -e "s/^.\{1,$(tput cols)\}$/ & /;ta" | tr -d '\n' | head -c $(tput cols); echo -e "\n"
    echo  "The Link below will open a SPECFEM3D Jupyter Notebook" | sed  -e :a -e "s/^.\{1,$(tput cols)\}$/ & /;ta" | tr -d '\n' | head -c $(tput cols); echo -e "\n"
    ...
    find $(pwd)/scoped/specfem3d -mindepth 1 -not -name "OUTPUT_FILES" -not -path "$(pwd)/scoped/specfem3d/OUTPUT_FILES/*" -exec rm -rf {} +
else
    exec $@
fi