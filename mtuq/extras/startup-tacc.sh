if [ $# -eq 0 ]; then
    ...
    echo "Welcome to the MTUQ for Tapis" | sed  -e :a -e "s/^.\{1,$(tput cols)\}$/ & /;ta" | tr -d '\n' | head -c $(tput cols); echo -e "\n"
    echo  "The Link below will open a MTUQ Jupyter Notebook" | sed  -e :a -e "s/^.\{1,$(tput cols)\}$/ & /;ta" | tr -d '\n' | head -c $(tput cols); echo -e "\n"
    ...
    NOTEBOOK_ARGS=" --notebook-dir=/home/scoped --port=${JUPYTER_PORT} --no-browser --ip=0.0.0.0 --allow-root --NotebookApp.token=${JUPYTER_PWD}"
    jupyter lab ${NOTEBOOK_ARGS}
else
    mkdir -p syngine_cache
    exec $@
fi
